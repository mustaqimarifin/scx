...
CREATE UNIQUE INDEX comment_reactions_user_id_comment_id_reaction_type_key ON public.comment_reactions USING btree (
    user_id,
    comment_id,
    reaction_type
);
...
or
replace
    view "public"."comment_reactions_metadata" as
SELECT
...
or
replace
    view "public"."comments_with_metadata" as
SELECT
    comments.id,
    comments.created_at,
    comments.topic,
...
CREATE policy "Enable delete for users based on user_id" on "public"."comment_reactions" as permissive for
delete
...
CREATE policy "Enable delete for users based on user_id" on "public"."comments" as permissive for
delete
    to public using ( (auth.uid() = user_id));
